...
public class SML_Align_Channels implements PlugIn {

	public static void main(final String... args) throws Exception {
		// create the ImageJ application context with all available services				
		final ij.ImageJ ij = new ij.ImageJ();
		ij.setVisible(true);
		Class<?> clazz = SML_Align_Channels.class;
	...
	public void run(String arg) { // macro inptut arg should be comma separated with window length and computation mode.		
		...
		if (computeModel != 5)
		{
			...
			try // in case no results table are available.
			{
				...
				correctDrift.ChannelAlign(parameters.chAlignShift, computeModel); // drift correct all channels.
			}
			finally
			{
