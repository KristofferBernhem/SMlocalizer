...
public class correctDrift_MultiCh {
...
/*		for (int i = 0; i < correctedResults.size(); i ++){
		...
		for(double Ch = 1; Ch <= Channels; Ch++){
			...
			double frameBin = Math.round( 				// Bin size for drift correction based on total number of frames and user input fraction. 
					...
					BinFrac[(int) Ch-1]);							// User input fraction.
			ArrayList<Particle> filteredResults =  new ArrayList<Particle>(); // output arraylist.
			int[] timeIndex = new int[(int) (Math.round(1.0/BinFrac[(int) Ch-1])+1)];
			double[] timeIndexDouble = new double[(int) (Math.round(1.0/BinFrac[(int) Ch-1])+1)];
			int count = 0;					
			...
			double[] lambdax = new double[(int) Math.round(1.0/BinFrac[(int) Ch-1])];								// Holds drift estimate between all bins in x.
			double[] lambday = new double[(int) Math.round(1.0/BinFrac[(int) Ch-1])];								// Holds drift estimate between all bins in y.
			double[] lambdaz = new double[(int) Math.round(1.0/BinFrac[(int) Ch-1])];								// Holds drift estimate between all bins in y.
			...
			for (int i = 1; i < timeIndex.length ; i++){ 												// Loop over all bins.
				if ((timeIndex[i] - timeIndex[i-1])<minParticles[(int) Ch-1]){									// If the bin lacks enough points to meet user minimum criteria.
					okBins++;				
				}
			}
			...
			if (okBins == 0){ 														// If all bins were ok.
				for (int i = 1; i < Math.round(1.0/BinFrac[(int) Ch-1]) ; i++){ 				// Loop over all bins.
					if(!ToFewReached){
						...
						for (int j = timeIndex[i]; j < timeIndex[i+1];j++){
							if (addedFrames1 < nParticles[(int) Ch-1] &&
						...
						for (int j = timeIndex[i-1]; j < timeIndex[i];j++){
							if (addedFrames2 < nParticles[(int) Ch-1] &&
					...
					//	System.out.println("Beta " + Data2.size() + " from round " + i);
						if(Alpha.size() < minParticles[(int) Ch-1] &&
								Beta.size() < minParticles[(int) Ch-1]){
							ij.IJ.log("not enough particles, no shift correction possible");
				...
				for (int j =  (int) (Math.round(1.0/BinFrac[(int) Ch-1]) - 1); j >0; j--){
