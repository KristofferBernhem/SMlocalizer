...
//					inputResults.get(i).photons > 0.8*inputResults.get(j).photons )
	...
	{		
		...
		for (int ch = 1; ch <= nChannels; ch++)
		{
			for (int i = 0; i < calibration.length; i++)
			{
				ij.Prefs.set("SMLocalizer.calibration.PRILM.xOffset.Ch"+ch+"."+i,0); // reset.
				ij.Prefs.set("SMLocalizer.calibration.PRILM.yOffset.Ch"+ch+"."+i,0); // reset.
			}
		}
		...
		for (int ch = 1; ch <= nChannels; ch++)
		{
			for (int i = 0; i < calibration.length; i++)
			{
				ij.Prefs.set("SMLocalizer.calibration.PRILM.xOffset.Ch"+ch+"."+i,xyOffset[0][i][ch-1]);
				ij.Prefs.set("SMLocalizer.calibration.PRILM.yOffset.Ch"+ch+"."+i,xyOffset[1][i][ch-1]);
			}
		}
		ij.Prefs.savePreferences(); // store settings.
...
//						if (inputParticle.get(idx).frame == center[ch] && inputParticle.get(idx).channel == ch+1)	// if we're at the center frame.
				...
				while (optimize)
				{
					...
					{
						double z = inputParticle.get(startIdx).z / ij.Prefs.get("SMLocalizer.calibration.PRILM.step",0);  // get back approximate frame.
...
/*
	...
	{
		double[][][] xyOffset = new double[2][(int) ij.Prefs.get("SMLocalizer.calibration.PRILM.height",0)][(int) ij.Prefs.get("SMLocalizer.calibration.PRILM.channels",0)]; // precast.
		for(int ch = 1; ch <= xyOffset[0][0].length; ch++) // load in correction table.
		{
			...
			{
				xyOffset[0][i][ch-1] =  ij.Prefs.get("SMLocalizer.calibration.PRILM.xOffset.Ch"+ch+"."+i,0);
				xyOffset[1][i][ch-1] =  ij.Prefs.get("SMLocalizer.calibration.PRILM.yOffset.Ch"+ch+"."+i,0);
			}
		}	
		double zStep =  ij.Prefs.get("SMLocalizer.calibration.PRILM.step",0);
