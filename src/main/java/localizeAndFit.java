...
public class localizeAndFit {
	...
	public static ArrayList<Particle> run(int[] MinLevel, int gWindow, int inputPixelSize,  int[] totalGain , int selectedModel, double maxSigma, String modality){						
		...
		{
			int	fitSize = (1+gWindow*gWindow+8)*4; // size of each fitThese entry.
			fitSize += 22*4; // size of each particle object, 
			fitSize += 23*4+gWindow*gWindow*4; // each fit calculation takes this space.
			int nMax = (int)(IJ.maxMemory()/fitSize);
			nMax /= 4;
			...
			for (int Ch = 1; Ch <= nChannels; Ch++)							// Loop over all channels.
			{
				...
				{											
					...
					if (Ch == nChannels && Frame == nFrames)
						loadedAll = true;
					if (fitThese.size() >= nMax || loadedAll)
					{
			...
			if (selectedModel == 1) // parallel
			{
				...
				{			
					...
					{						
						...
						long maxMemoryGPU = (long) (0.50*free[0]); 
